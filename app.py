...
parametros = json.load(open("parametros_modelo.json"))

coef_dict = dict(zip(importancias["variable"], importancias["coeficiente"]))

def predecir(precio_base, duracion, proveedores_invitados, respuestas_al_procedimiento,
             modalidad_de_contratacion, departamento_entidad):
    z = parametros["intercepto"]
    valores_num = {
    ...
    for var, valor in valores_num.items():
        media = parametros["numericas"][var]["media"]
        escala = parametros["numericas"][var]["escala"]
        z += coef_dict.get(var, 0) * ((valor - media) / escala)

    z += coef_dict.get(f"modalidad_de_contratacion_{modalidad_de_contratacion}", 0)
    z += coef_dict.get(f"departamento_entidad_{departamento_entidad}", 0)
...
if st.button("Predecir"):
    prob = predecir(precio_base, duracion, proveedores_invitados, respuestas_al_procedimiento,
                     modalidad_de_contratacion, departamento_entidad)
